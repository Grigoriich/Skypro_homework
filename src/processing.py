...
def sort_by_date(dictionary_list: list[dict], reverse_: bool=False) -> list[dict]:
    '''Функция, сортирующая список словарей по дате, с возможностью изменения порядка сортировки'''
    result_list = sorted(dictionary_list, reverse=reverse_, key=lambda x: get_date_format(x["date"]))
    return result_list
